...
Также в некоторых ситуация можно использовать сокращение, которое будет возвращать не значение "Порядок", а значение "Ссылка". Работает это так, что данные сокращения будут преобразовываться в подзапрос. Прошу обратить внимание, что в кострукции выглядит именно **[$$...]**, с двойным символом доллара. Пример:
```sql
-- [$$Перечисление.ФормыОплаты.Наличная]
(select top 1 _KeyField from _Enum1273 where _EnumOrder = 1)
-- [$$БизнесПроцесс.Задание.ТочкиМаршрута.Выполнить]
(select top 1 _KeyField from _BPrPoints1298 where _RoutePointOrder = 3)
```
